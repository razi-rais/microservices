...
```kubernetes.pod_name , 	kubernetes.namespace_name, log and @timestamp  ``` fields. You will see the pane on the right side showing the results now have four columns added to it. Also, click on the ```@timestamp`` column to make it sort in decending order (latest logs first) 
